...
function  fuzz_mode(){
    httrack ${url} -O temp -N "%h%p/%n/%t" -v > /dev/null 2>&1 
    grep -oE "href=\".+/\"" temp/index.html 
...
if [[ $parameter_counter -ne  2 ]];then  # check  if  parameter_counter  is correct igualment two parameters passed in the script.
    helpPanel
else
    fuzz_mode  
fi 
